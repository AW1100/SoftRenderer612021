...
DeviceContext::DeviceContext() :m_renderMode(WIREFRAME), m_cameraPos(Vec4(0.0, 0.0, 0.0, 1.0))
{
}
...
//设置顶点缓冲
void DeviceContext::setVertexBuffer(std::vector<VertexIn> vertices)
{
	m_vertices = vertices;
}

//设置相机位置
void DeviceContext::setCameraPos(const Vec4& pos)
{
	m_cameraPos = pos;
}

//设置索引缓冲
void DeviceContext::setIndexBuffer(std::vector<UINT> indices)
{
	m_indices = indices;
}

//设置着色器
void DeviceContext::setShader(ShaderBase* base)
{
	m_pShader = base;
}
...
bool DeviceContext::backFaceCulling(const VertexIn& v1, const VertexIn& v2, const VertexIn& v3)
{
	...
	else
	{
		...
		Vec4 viewDir = v1.pos - m_cameraPos;
